            ...
            if num_of_docs > 0:
                ...
                for hit in result['hits']['hits']:
                    if hit['_source'].get('fid')!= None and hit['_source'].get('location_on_disk')!= None:
                        fid_list.append(hit['_source']['fid'] + ' , ' + hit['_source']['location_on_disk'] ) 
...
def get_host():
    ##set is_docker = True when run docker image
    ##set is_docker = False when run on local machine
    ##the host string is dependent on where app is run
    ##
    is_docker = False
    host = 'localhost'
    if is_docker:
