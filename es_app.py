...
def login():
    ...
    if not(username is None or password is None) and form.connect.data: 
        ...
        #"host": "localhost"
        es = Elasticsearch(hosts=[{"host": get_host(), "port": 9200}], http_auth=(username, password))
        if es.ping():
...
def create():
    ...
    if form.delete_document.data:
        ...
        document_selection = form.documents.data
        document_to_be_deleted = document_selection.split(',')[0].strip()
        ...
        if document_to_be_deleted == "No Document in Selected Index":
            document_message = "No document has been deleted";
        else:
            ...
            #     fid_list = ["No Document in Selected Index"]
            form.documents.choices = fid_list  
            document_message = 'Document with file id ' + document_to_be_deleted + ' has been deleted'
            
        ##update'
        return render_template('create.html', form = form, \
            message_document= document_message, message_index = msg)
...
def docs(index):
    ...
    password = session.get('password','password')
    es = Elasticsearch(hosts=[{"host": get_host(), "port": 9200}], http_auth=(username, password)) 
    msg, d_list, l_list = query_all(es, index)
    docs_array =[]
    #d_list = ['abc','123']
    for d in d_list:
        docs_obj = {}
        docs_obj['fid'] = d
        docs_obj['msg'] = msg
        docs_array.append(docs_obj)
    return jsonify({'docu:' :docs_array})
